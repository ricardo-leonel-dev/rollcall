...
@Component({
  ...
  styles: [`
    ...
    .icon-box.primary { background: var(--accent-soft); color: var(--accent); }
    .title { font-family: 'Nunito', sans-serif; font-size: 19px; font-weight: 700; color: var(--ink); margin: 0 0 6px; }
    .message { color: var(--ink-soft); font-size: 14px; line-height: 1.5; white-space: pre-line; }
  ...
  template: `
    ...
    <div class="header">
      <div class="icon-box" [class.primary]="data.severity === 'primary'"><mat-icon>{{data.icon ?? 'delete_outline'}}</mat-icon></div>
      <div>
    ...
    <div class="actions">
      ...
      <button mat-flat-button [color]="data.severity === 'primary' ? 'primary' : 'warn'" [mat-dialog-close]="true">{{data.confirmLabel ?? 'Eliminar'}}</button>
    </div>
  `,
})
